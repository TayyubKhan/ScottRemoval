...
import 'package:scotremovals/repository/dropoutrepo.dart';
import 'package:scotremovals/repository/home_repo.dart';
import 'package:scotremovals/repository/image.dart';
import 'package:scotremovals/repository/orderDetailsRepo.dart';
import 'package:scotremovals/repository/pickuprepo.dart';
import 'package:scotremovals/repository/withourmodel_repo.dart';
...
class _Order_DetailState extends State<Order_Detail> {
  ...
  void initState() {
    ...
    super.initState();
    final google = Provider.of<GoogleMapViewModel>(context, listen: false);
    final data = Provider.of<DataViewViewModel>(context, listen: false);
    google.isfetched = false;
    final order = OrderDetailRepo();
    int index = int.parse(data.data[3]);
    order.orderDetailApi(context).then((value) {
      data.getdata(
          value['waiver_form'][value['waiver_form'].length - 1]
                  ['waiver_description']
              .toString(),
          index);
      data.getdata2(
          value['comments'][value['comments'].length - 1]['messege'].toString(),
          index);
    });
  }
  ...
  bool _isPolylinesFetched = false;
                ...
                builder: (BuildContext context, snapshot) {
                  if (snapshot.hasData) {
                    ...
                    if (snapshot.data!.data![index].productsList!
                        .toString()
                        .isNotEmpty) {
                      dt = List<Map<String, dynamic>>.from(jsonDecode(snapshot
                    ...
                    for (int j = 0; j < dataList.length; j++) {
                      total = total + int.parse(dataList[j]['quantity']);
                    }
                    return SizedBox(
                        ...
                        children: [
                          SizedBox(
                            ...
                            child: Stack(
                              ...
                              children: [
                                Consumer<GoogleMapViewModel>(
                                    builder: (context, value, _) {
                                  try{value.getLng(
                                      LatLng(
                                          double.parse(snapshot.data!
                                              .data![index].pickupLatitude!),
                                          double.parse(snapshot.data!
                                              .data![index].pickupLongitude!)),
                                      LatLng(
                                          double.parse(snapshot.data!
                                              .data![index].dropLatitude!),
                                          double.parse(snapshot.data!
                                              .data![index].dropLongitude!)));}
                                  catch(e){
                                    value.getLng(
                                        const LatLng(0.00, 0.00),
                                        const LatLng(0.00, 0.00));
                                  }
                                  return GoogleMap(
                                      key: _uniqueKey,
                                      ...
                                      initialCameraPosition: CameraPosition(
                                          target: LatLng(
                                              value.startlatlng.latitude,
                                              value.startlatlng.longitude),
                                          zoom: 10),
                                  ...
                                  return Padding(
                                    ...
                                    child: Row(
                                      ...
                                      children: [
                                        ...
                                        InkWell(
                                          onTap: () async {
                                            value.setAPIData(dataList);
                                            dialogBox(context, value);
